...
const Reviews = () => {
  ...
  const reviewsData = t.reviews?.items || [];
  const half = Math.ceil(reviewsData.length / 2);
  const row1 = reviewsData.slice(0, half);
  const row2 = reviewsData.slice(half);

  const ReviewCard = ({ review }: { review: any }) => (
    <div className="w-[350px] md:w-[420px] flex-shrink-0 p-8 rounded-2xl border border-white/5 bg-[#050505] shadow-2xl flex flex-col justify-between" style={{ minHeight: "260px" }}>
      <div>
        <div className="text-5xl text-amber-600/40 font-serif leading-none mb-2">“</div>
        <p className="text-slate-300 leading-relaxed text-sm md:text-base">"{review.quote}"</p>
      </div>
      <div className="flex items-center gap-4 mt-8">
        <div className="w-12 h-12 rounded-full overflow-hidden relative border border-white/10 bg-gradient-to-br from-zinc-800 to-black flex items-center justify-center">
           <span className="text-white/60 font-bold">{review.name.charAt(0)}</span>
        </div>
        <div className="flex-1">
          <h4 className="text-white font-bold text-sm md:text-base">{review.name}</h4>
          <p className="text-slate-500 text-xs md:text-sm">{review.title}</p>
        </div>
  ...
  return (
    <section id="reviews" className="relative z-10 py-24 overflow-hidden bg-black/20">
      <div className="text-center mb-16 px-6 relative z-20">
        <h2 className="text-4xl font-bold mb-4 tracking-tight">{t.reviews.title}</h2>
        <p className="text-slate-400">{t.reviews.subtitle}</p>
      </div>
      
      <div className="relative w-full">
        {/* Fades for left and right edges */}
